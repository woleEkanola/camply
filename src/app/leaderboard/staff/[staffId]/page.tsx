...
import { ArrowLeftIcon, TrophyIcon, StarIcon, SparklesIcon, ClockIcon } from "@heroicons/react/24/outline";
import { leaderboardArea } from "@/components/leaderboard/area";

/**
 * Staff (teacher/volunteer) detail — `staffDetail` has existed in the router
 * since PR7 but had no page. Same shape as the camper detail page, plus the
 * 0-5 composite rating (see aggregate.ts's computeCompositeScores).
 */
export default function StaffDetailPage({ params }: { params: Promise<{ staffId: string }> }) {
  ...
  const { data, isLoading } = api.leaderboard.staffDetail.useQuery({ campId: campId!, staffId }, { enabled: !!campId });
    ...
    <AppShell area={area}>
      <div className="mx-auto max-w-5xl space-y-6 pb-12">
        ...
        {isLoading || !data ? (
          ...
          <>
            {/* Overview */}
            <div className="relative overflow-hidden rounded-xl px-6 py-8 text-center shadow-sm" style={{ backgroundColor: "#6D4C41" }}>
              <div className="absolute inset-0 bg-black/25" />
              <div className="relative">
                {data.staff.photoUrl && (
                  <img src={data.staff.photoUrl} alt="" className="mx-auto mb-3 h-16 w-16 rounded-full border-2 border-white/60 object-cover" />
                )}
                <h1 className="text-2xl font-extrabold tracking-wide text-white">
                  {data.staff.firstName} {data.staff.lastName}
                </h1>
                <p className="mt-1 text-sm text-white/80">{data.staff.type}</p>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <StatCard
                label="Teacher Score"
                value={typeof data.stat?.compositeScore === "number" ? `${data.stat.compositeScore.toFixed(1)} / 5` : "—"}
                insight="Blended rating"
                tone="success"
