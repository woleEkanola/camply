...
export default function TribesConfigPage() {
  ...
  const yearId = typeof params.id === "string" ? params.id : "";
  useSession({ required: true, onUnauthenticated: () => router.push("/login") });

  const utils = api.useUtils();
  const { data: year } = api.year.getById.useQuery({ id: yearId }, { enabled: !!yearId });
  const { data: tribes, refetch: refetchTribes } = api.tribe.listByYear.useQuery({ yearId }, { enabled: !!yearId });
  ...
  useEffect(() => {
    if (!year) return;
    setEnabled((year as any).tribeAllocationEnabled ?? false);
    setMode((year as any).tribeAllocationMode ?? "MANUAL");
  ...
  const saveConfig = api.tribe.updateAllocationConfig.useMutation({
    onSuccess: () => utils.year.getById.invalidate({ id: yearId }),
  });
  ...
  return (
    <AppShell area="admin">
      <div className="mx-auto max-w-4xl space-y-6">
        <PageHeader title={`Tribes: ${year?.name ?? ""}`} />
        ...
        <Card>
          ...
          <CardBody className="space-y-4">
            ...
            <Button
              ...
              onClick={() => saveConfig.mutate({ yearId, tribeAllocationEnabled: enabled, tribeAllocationMode: mode, tribeAllocationRules: rules })}
            >
        ...
        <Card>
          ...
          <CardBody>
            ...
            <Button loading={bulkAssign.isPending} onClick={() => bulkAssign.mutate({ yearId })}>Run Bulk Auto-Allocation</Button>
          </CardBody>
        </Card>
      </div>
    </AppShell>
