...
import { ClipboardDocumentCheckIcon, StarIcon, UserGroupIcon, UsersIcon } from "@heroicons/react/24/outline";

type HubTab = "OVERVIEW" | "CAMPERS" | "TEACHERS" | "ATTENDANCE" | "POINTS";
...
export function TribeHub({ campId, organizationId, admin = false }: { campId: string; organizationId: string; admin?: boolean }) {
  ...
  return <div className="space-y-5" data-testid="tribe-hub">
    <div className="overflow-hidden rounded-2xl border border-border-default bg-surface">
      <div className="p-5 text-white" style={{ background: `linear-gradient(135deg, ${tribe.color ?? "#6D4C41"}, ${tribe.color ?? "#6D4C41"}bb)` }}>
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-widest text-white/70">Tribe operations hub</p><h2 className="mt-1 text-2xl font-black">{tribe.name}</h2><p className="mt-1 text-sm text-white/80">{tribe.motto || tribe.meaning || "Attendance, people and points in one place."}</p></div>{admin && <Select aria-label="Choose tribe" value={tribe.id} onChange={(event) => { setTribeId(event.target.value); setTab("OVERVIEW"); }} className="min-w-52 bg-white text-neutral-900">{data.tribes.map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select>}</div>
      </div>
