...
export default function AppShell({ area, children }: AppShellProps) {
  ...
  const sidebarContent = (
    <>
      <div className="flex h-14 items-center justify-between px-4">
        <span className={cn("truncate font-semibold text-neutral-900 dark:text-white", !sidebarOpen && "hidden")}>
          {organization?.name || "Camply"}
        </span>
        <button
          onClick={() => setSidebarOpen((v) => !v)}
          className="hidden rounded-md p-1.5 text-neutral-400 hover:bg-neutral-100 hover:text-neutral-600 dark:hover:bg-neutral-800 dark:hover:text-neutral-200 md:block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500"
        ...
        <button
          ...
          className="rounded-md p-1.5 text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 md:hidden"
      ...
      >
        {groups.map((group) => (
          <div key={group.name} className="mb-4">
            <div className={cn("mb-1 px-3 text-xs font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500", !sidebarOpen && "hidden")}>
              {group.name}
            </div>
            <div className="space-y-0.5">
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={cn("h-5 w-5 shrink-0", active ? "text-accent-600 dark:text-accent-400" : "text-neutral-400 dark:text-neutral-500")} aria-hidden="true" />
      ...
      <div className="border-t border-neutral-200 dark:border-neutral-800 p-2">
        <button
          ...
          className="flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 hover:text-neutral-900 dark:hover:text-white"
        >
          <ArrowRightOnRectangleIcon className="h-5 w-5 shrink-0 text-neutral-400 dark:text-neutral-500" aria-hidden="true" />
  ...
  return (
    <div className="flex h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100">
      ...
      <div
        className={cn(
          "hidden md:flex md:flex-col border-r border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 transition-all duration-200",
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-xs" onClick={() => setMobileOpen(false)} aria-hidden="true" />
          <div className="fixed inset-y-0 left-0 flex w-72 flex-col bg-white dark:bg-neutral-900 shadow-xl">{sidebarContent}</div>
        </div>
      )}

      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-4 pt-[env(safe-area-inset-top)]">
          <button
            onClick={() => setMobileOpen(true)}
            className="rounded-md p-1.5 text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 md:hidden"
          ...
          <button
            ...
            className="hidden items-center gap-2 rounded-md border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-3 py-1.5 text-sm text-neutral-400 hover:border-neutral-300 dark:hover:border-neutral-700 hover:text-neutral-500 md:flex"
          >
            Search...
            <kbd className="rounded border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800 px-1.5 py-0.5 text-xs text-neutral-500 dark:text-neutral-400">⌘K</kbd>
          </button>
