...
export function CampAttendancePanel({ campId, organizationId, access, lockedTribeId }: { campId: string; organizationId: string; access: any; lockedTribeId?: string }) {
  ...
  const deleteRecord = api.attendance.deleteRecord.useMutation({ onSuccess: () => { setMessage("Attendance record deleted."); refresh(); }, onError: fail });
  const reopenSession = api.attendance.reopenSession.useMutation({ onSuccess: () => { setMessage("Session reopened."); refresh(); }, onError: fail });
  const deleteSession = api.attendance.deleteSession.useMutation({ onSuccess: () => { setMessage("Session deleted."); setActiveSessionId(null); refresh(); }, onError: fail });
  const clearAutoAbsent = api.attendance.clearAutoAbsent.useMutation({ onSuccess: () => { setMessage("Auto-absent sweep cleared."); refresh(); }, onError: fail });

  function promptAndDeleteRecord(recordId: string, name: string) {
    const reason = window.prompt(`Delete ${name}'s attendance record?\n\nEnter a reason (required) — any points it awarded will be voided.`);
    if (reason === null) return;
    if (!reason.trim()) { setError("A reason is required to delete an attendance record."); return; }
    deleteRecord.mutate({ sessionId: activeSession.id, recordId, staff: audience !== "CAMPER", reason: reason.trim() });
  }
  function promptAndDeleteSession() {
    const reason = window.prompt(`Delete the entire "${activeSession.name}" session?\n\nEnter a reason (required) — every record and the points it awarded will be removed.`);
    if (reason === null) return;
    if (!reason.trim()) { setError("A reason is required to delete a session."); return; }
    deleteSession.mutate({ sessionId: activeSession.id, reason: reason.trim() });
  }
  function promptAndClearAutoAbsent() {
    const reason = window.prompt(`Clear the "mark remaining absent" sweep for "${activeSession.name}"?\n\nOnly rows auto-created by Close (not manually marked absent) are removed. Enter a reason (required).`);
    if (reason === null) return;
    if (!reason.trim()) { setError("A reason is required to clear the auto-absent sweep."); return; }
    clearAutoAbsent.mutate({ sessionId: activeSession.id, reason: reason.trim() });
  }
  ...
  return <div className="space-y-5" data-testid="camp-attendance-panel">
    ...
    {activeSession && <>
      ...
      <Card><CardBody>
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h3 className="font-semibold text-txt-primary">{activeSession.name}</h3><p className="text-xs text-txt-secondary">QR, search, and manual marks update one register and the leaderboard.</p></div><div className="flex flex-wrap gap-2"><Button disabled={activeSession.status !== "OPEN"} onClick={() => setScannerOpen(true)}>Scan QR</Button><Button variant="danger" disabled={activeSession.status !== "OPEN"} loading={close.isPending} onClick={() => window.confirm("Close this session and mark everyone not recorded as absent?") && close.mutate({ sessionId: activeSession.id, markRemainingAbsent: true })}>Close</Button>{access.isAdmin && activeSession.status === "CLOSED" && <Button variant="secondary" loading={reopenSession.isPending} onClick={() => window.confirm("Reopen this closed session so marks can be corrected?") && reopenSession.mutate({ sessionId: activeSession.id })}>Reopen</Button>}{access.isAdmin && <Button variant="secondary" loading={clearAutoAbsent.isPending} onClick={promptAndClearAutoAbsent}>Clear auto-absent</Button>}{access.isAdmin && <Button variant="danger" loading={deleteSession.isPending} onClick={promptAndDeleteSession}>Delete session</Button>}</div></div>
        ...
        <div className="divide-y divide-border-subtle">{roster.map((person: any) => { const current = marks.get(person.id); const name = audience === "CAMPER" ? person.camper.name : `${person.preferredName || person.firstName} ${person.lastName}`.trim(); return <div key={person.id} data-testid="attendance-roster-row" className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between"><div><span className="text-sm font-medium text-txt-primary">{name}</span>{audience !== "CAMPER" && <span className="ml-2 text-xs text-txt-muted">{person.type}</span>}{current && <div className="mt-1"><Badge tone={current.status === "PRESENT" ? "success" : current.status === "LATE" ? "warning" : "neutral"}>{current.status}</Badge></div>}</div><div className="flex flex-wrap items-center gap-1">{STATUSES.map((status) => <Button key={status} size="sm" variant={current?.status === status ? "primary" : "secondary"} disabled={activeSession.status !== "OPEN"} onClick={() => mark.mutate({ sessionId: activeSession.id, ...(audience === "CAMPER" ? { registrationId: person.id } : { staffProfileId: person.id }), status, source: "MANUAL" })}>{status[0] + status.slice(1).toLowerCase()}</Button>)}{access.isAdmin && current && <button type="button" data-testid="delete-attendance-record" className="ml-1 text-xs text-status-danger underline disabled:opacity-50" disabled={deleteRecord.isPending} onClick={() => promptAndDeleteRecord(current.id, name)}>Delete</button>}</div></div>; })}</div>
      </CardBody></Card>
    </>}
