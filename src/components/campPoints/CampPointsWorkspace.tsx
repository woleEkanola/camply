...
export function CampPointsWorkspace({ campId, organizationId }: { campId: string; organizationId: string }) {
  ...
  const rosterInput = { campId, tribeId: effectiveTribeId || undefined, campusId: effectiveCampusId || undefined };
  ...
  const { data: history = [] } = api.campPoints.history.useQuery({ campId, tribeId: access?.isAdmin && awardGroupType === "TRIBE" ? tribeId || undefined : undefined, campusId: access?.isAdmin && awardGroupType === "CAMPUS" ? campusId || undefined : undefined }, { enabled: !!access });
  ...
  const startStation = () => {
    ...
    start.mutate({ campId, categoryId: category.id, tribeId: awardGroupType === "TRIBE" ? tribeId : undefined, campusId: awardGroupType === "CAMPUS" ? campusId : undefined, ...(access.isAdmin ? { points: Number(points) } : {}) });
  };
  ...
  return <div className="space-y-5" data-testid="camp-points-workspace">
    ...
    {tab === "AWARD" && batch && <div className="space-y-4">
      ...
      <Card><CardBody><div className="mb-3 flex items-center justify-between"><div><h3 className="font-semibold text-txt-primary">Select several teenagers</h3><p className="text-xs text-txt-secondary">Choose any number, review them, then award together.</p></div><Button disabled={!selected.size} loading={award.isPending} onClick={() => award.mutate({ batchId: batch.id, registrationIds: [...selected], entryMethod: "SELECT" })}>Award {selected.size}</Button></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{roster.map((registration: any) => <label key={registration.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-border-default p-3 has-[:checked]:border-accent-500 has-[:checked]:bg-accent-500/5"><input type="checkbox" checked={selected.has(registration.id)} onChange={() => toggle(registration.id)} /><span><span className="block text-sm font-medium text-txt-primary">{registration.camper.name}</span><span className="text-xs text-txt-muted">{registration.registrationNumber}</span></span></label>)}</div></CardBody></Card>
    </div>}

    {tab === "HISTORY" && <Card><CardBody><h3 className="mb-4 font-semibold text-txt-primary">Recent camper point activity</h3>{!history.length ? <p className="text-sm text-txt-muted">No point activity yet.</p> : <div className="divide-y divide-border-subtle">{history.map((event: any) => <div key={event.id} className="flex items-center justify-between gap-3 py-3"><div><div className="text-sm font-medium text-txt-primary">{event.camperName}</div><div className="text-xs text-txt-secondary">{event.category?.name ?? event.reason ?? "Points"} · {new Date(event.occurredAt).toLocaleString()}</div></div><div className="flex items-center gap-2"><span className={`font-bold ${event.points < 0 ? "text-danger-600" : "text-success-600"}`}>{event.points > 0 ? "+" : ""}{event.points}</span>{event.points > 0 && event.source === "MANUAL" && <Button size="sm" variant="secondary" onClick={() => undo.mutate({ eventId: event.id })}>Undo</Button>}</div></div>)}</div>}</CardBody></Card>}

    <Dialog open={!!category && !batch} onClose={() => setCategory(null)} title={category ? `Open ${category.name}` : "Point station"} size="sm"><div className="space-y-4"><p className="text-sm text-txt-secondary">Every teenager scanned or selected will receive this award once in the session.</p><Input label="Points" type="number" value={points} onChange={(event) => setPoints(event.target.value)} disabled={!access.isAdmin} /><div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setCategory(null)}>Cancel</Button><Button disabled={!scopeReady || !Number(points)} loading={start.isPending} onClick={startStation}>Open station</Button></div></div></Dialog>
