...
import { ArchiveBoxIcon, ArrowRightIcon, BuildingOffice2Icon, ExclamationTriangleIcon, ListBulletIcon, PlusIcon, RectangleGroupIcon, SparklesIcon, TrashIcon } from "@heroicons/react/24/outline";

type ReconciliationAction = "MERGE" | "KEEP" | "ARCHIVE";
type ReconciliationDecision = { action: ReconciliationAction; targetId?: string };
...
export function DepartmentsWorkspace({ organizationId, campId, canManageAll = false, staffArea = false }: { organizationId: string; campId: string; canManageAll?: boolean; staffArea?: boolean }) {
  ...
  const deleteDepartment = api.department.delete.useMutation({
    onSuccess: () => { setDeleteTarget(null); toast.success("Department deleted. It can be restored from Trash for 60 days."); utils.departmentOperations.list.invalidate(); },
    onError: (error) => toast.error(error.message),
  });
  const archiveDepartment = api.department.archive.useMutation({
    onSuccess: () => { setArchiveTarget(null); toast.success("Department archived. Toggle \"Include inactive\" to see it again."); utils.departmentOperations.list.invalidate(); },
    onError: (error) => toast.error(error.message),
  });
  const bulkArchive = api.department.bulkArchive.useMutation({
    onSuccess: (result) => {
      const failed = result.results.filter((item) => !item.ok);
      toast[failed.length ? "error" : "success"](failed.length ? `${failed.length} of ${result.results.length} couldn't be archived.` : `${result.results.length} department(s) archived.`);
      setSelectedIds([]);
      utils.departmentOperations.list.invalidate();
    },
    onError: (error) => toast.error(error.message),
  });
  const bulkDelete = api.department.bulkDelete.useMutation({
    onSuccess: (result) => {
      const failed = result.results.filter((item) => !item.ok);
      toast[failed.length ? "error" : "success"](failed.length ? `${failed.length} of ${result.results.length} couldn't be deleted — clear their people, roles, and child departments first, or reconcile them.` : `${result.results.length} department(s) deleted. Restorable from Trash for 60 days.`);
      setSelectedIds([]);
      utils.departmentOperations.list.invalidate();
    },
    onError: (error) => toast.error(error.message),
  });
  const reconciliationPlan = api.departmentOperations.reconciliationPlan.useQuery({ campId }, { enabled: canManageAll && !!campId });
  const applyReconciliation = api.departmentOperations.applyReconciliation.useMutation({
    onSuccess: (result) => {
      const failed = result.results.filter((item) => !item.ok);
      if (failed.length) toast.error(`${failed.length} of ${result.results.length} decisions couldn't be applied. Review and try again.`);
      else toast.success("Departments reconciled with the 2026 JD.");
      setReconcileOpen(false);
      utils.departmentOperations.list.invalidate();
      utils.departmentOperations.reconciliationPlan.invalidate();
    },
    onError: (error) => toast.error(error.message),
  });
  ...
  return <div className="space-y-6 pb-24">
    ...
    {view === "departments" && !staffArea && <div className="space-y-6">
      ...
      {canManageAll && (reconciliationPlan.data?.unmatched.length ?? 0) > 0 && <div className="flex flex-col gap-2 rounded-lg border border-warning-200 bg-warning-50 p-3 text-sm text-warning-800 sm:flex-row sm:items-center sm:justify-between">
        <span className="flex items-start gap-2"><ExclamationTriangleIcon className="mt-0.5 h-5 w-5 shrink-0" /><span>{reconciliationPlan.data!.unmatched.length} department{reconciliationPlan.data!.unmatched.length === 1 ? " isn't" : "s aren't"} part of the 2026 JD — review and reconcile {reconciliationPlan.data!.unmatched.length === 1 ? "it" : "them"}.</span></span>
        <Button variant="secondary" onClick={() => setReconcileOpen(true)}>Review</Button>
      </div>}
      ...
      {query.isLoading ? <div className="py-12 text-center text-sm text-txt-secondary">Loading departments…</div> : groups.length === 0 ? <EmptyState title="No departments found" description={canManageAll ? "Install the JD structure or create a department manually." : "No active departments match these filters."} /> : layout === "cards" ? groups.map(([group, items]) => <section key={group} className="space-y-3"><h3 className="text-sm font-semibold uppercase tracking-wide text-txt-secondary">{group}</h3><div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">{items.map((department) => <DepartmentCard key={department.id} department={department} canManage={canManageAll} onOpen={() => canManageAll ? router.push(`/admin/departments/${department.id}`) : changeView("contacts")} onArchive={() => setArchiveTarget({ id: department.id, name: department.name })} onDelete={() => setDeleteTarget({ id: department.id, name: department.name })} />)}</div></section>) : <>
        ...
        <Table
          ...
          onRowClick={(department) => canManageAll ? router.push(`/admin/departments/${department.id}`) : changeView("contacts")}
          selectable={canManageAll}
          selectedIds={selectedIds}
          onSelectionChange={setSelectedIds}
          actions={canManageAll ? (department) => department.systemKey === "CAMP_COMMAND" ? null : <div className="flex items-center gap-1"><button type="button" aria-label={`Archive ${department.name}`} onClick={(event) => { event.stopPropagation(); setArchiveTarget({ id: department.id, name: department.name }); }} className="rounded-lg p-1.5 text-txt-muted hover:bg-warning-50 hover:text-warning-700"><ArchiveBoxIcon className="h-4 w-4" /></button><button type="button" aria-label={`Delete ${department.name}`} onClick={(event) => { event.stopPropagation(); setDeleteTarget({ id: department.id, name: department.name }); }} className="rounded-lg p-1.5 text-txt-muted hover:bg-danger-50 hover:text-danger-700"><TrashIcon className="h-4 w-4" /></button></div> : undefined}
        />
      </>}
    </div>}
    ...
    <Dialog open={!!archiveTarget} onClose={() => setArchiveTarget(null)} title="Archive department" size="sm"><div className="space-y-4"><p className="text-sm text-txt-secondary">Archive <strong className="text-txt-primary">{archiveTarget?.name}</strong>? It stops appearing in the active list but stays fully intact — toggle &quot;Include inactive&quot; to find and unarchive it later.</p><div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setArchiveTarget(null)}>Cancel</Button><Button variant="danger" loading={archiveDepartment.isPending} onClick={() => archiveTarget && archiveDepartment.mutate({ id: archiveTarget.id })}>Archive department</Button></div></div></Dialog>
    <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete department" size="sm"><div className="space-y-4"><p className="text-sm text-txt-secondary">Delete <strong className="text-txt-primary">{deleteTarget?.name}</strong>? It can be restored from Trash for 60 days.</p><p className="text-xs text-txt-muted">Clear active people, current role assignments, and child departments first — or merge it into another department from the reconciliation review above.</p><div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setDeleteTarget(null)}>Cancel</Button><Button variant="danger" loading={deleteDepartment.isPending} onClick={() => deleteTarget && deleteDepartment.mutate({ id: deleteTarget.id })}>Delete department</Button></div></div></Dialog>
    <ReconciliationDialog open={reconcileOpen} onClose={() => setReconcileOpen(false)} plan={reconciliationPlan.data} loading={applyReconciliation.isPending} onApply={(decisions) => applyReconciliation.mutate({ campId, decisions })} />
  </div>;
}

const departmentListColumns: Column<any>[] = [
  { id: "name", header: "Department", accessor: (department) => department.name, primary: true },
  { id: "reportsTo", header: "Reports to", accessor: (department) => department.parentDepartment?.name ?? "Camp leadership" },
  { id: "leader", header: "Leader", accessor: (department) => leaderName(department, "HEAD") },
  { id: "assistant", header: "Assistant", accessor: (department) => leaderName(department, "ASSISTANT_HEAD") },
  { id: "peopleRoles", header: "People / roles", accessor: (department) => `${department._count.staff} / ${department._count.positions}` },
  { id: "today", header: "Today", accessor: (department) => `${department.today.completed}/${department.today.total} · ${department.today.completionPct}%` },
  { id: "status", header: "Status", accessor: (department) => <Badge tone={department.status === "ACTIVE" ? "success" : "neutral"}>{department.status}</Badge> },
];

type ReconciliationPlan = {
  matched: { id: string; name: string }[];
  unmatched: { id: string; name: string; staffCount: number; preferredByCount: number; currentPositionCount: number; childCount: number; checklistItemCount: number; suggestedTargetId: string | null; suggestedTargetName: string | null; confidence: "HIGH" | "MEDIUM" | "LOW" | "NONE" }[];
};

function ReconciliationDialog({ open, onClose, plan, loading, onApply }: { open: boolean; onClose: () => void; plan: ReconciliationPlan | undefined; loading: boolean; onApply: (decisions: { sourceId: string; action: ReconciliationAction; targetId?: string }[]) => void }) {
  ...
  useEffect(() => {
    ...
    setDecisions((previous) => {
      ...
      for (const item of plan.unmatched) {
        if (!next[item.id]) next[item.id] = item.suggestedTargetId ? { action: "MERGE", targetId: item.suggestedTargetId } : { action: "KEEP" };
      }
      return next;
    });
  }, [plan]);
  ...
  if (!plan) return null;
  const mergeItems = plan.unmatched.filter((item) => (decisions[item.id]?.action ?? "KEEP") === "MERGE");
  const movedStaff = mergeItems.reduce((sum, item) => sum + item.staffCount, 0);
  const invalid = mergeItems.some((item) => !decisions[item.id]?.targetId);

  function setDecision(id: string, decision: ReconciliationDecision) {
    setDecisions((previous) => ({ ...previous, [id]: decision }));
  }

  return <Dialog open={open} onClose={onClose} title="Reconcile departments with the 2026 JD" size="lg">
    <div className="space-y-4">
      <p className="text-sm text-txt-secondary">These departments aren&apos;t part of the installed JD structure. Choose what happens to each — nothing is written until you apply.</p>
      <div className="max-h-[50vh] space-y-3 overflow-y-auto">
        {plan.unmatched.map((item) => {
          const decision = decisions[item.id] ?? { action: "KEEP" as const };
          return <div key={item.id} className="space-y-2 rounded-xl border border-border-default p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-txt-primary">{item.name}</p>
                <p className="text-xs text-txt-secondary">{item.staffCount} active staff · {item.preferredByCount} preferred it · {item.currentPositionCount} roles · {item.checklistItemCount} checklist items{item.childCount ? ` · ${item.childCount} child department${item.childCount === 1 ? "" : "s"}` : ""}</p>
              </div>
              {item.suggestedTargetId && <Badge tone={item.confidence === "HIGH" ? "success" : item.confidence === "MEDIUM" ? "warning" : "neutral"}>{item.confidence.toLowerCase()} match</Badge>}
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Select aria-label={`Action for ${item.name}`} value={decision.action} onChange={(event) => { const action = event.target.value as ReconciliationAction; setDecision(item.id, { action, targetId: action === "MERGE" ? decision.targetId ?? item.suggestedTargetId ?? plan.matched[0]?.id : undefined }); }}>
                <option value="MERGE">Merge into…</option>
                <option value="KEEP">Keep as-is</option>
                <option value="ARCHIVE">Archive</option>
              </Select>
              {decision.action === "MERGE" && <Select aria-label={`Merge target for ${item.name}`} value={decision.targetId ?? ""} onChange={(event) => setDecision(item.id, { action: "MERGE", targetId: event.target.value })}>
                <option value="">Choose a department</option>
                {plan.matched.map((target) => <option key={target.id} value={target.id}>{target.name}</option>)}
              </Select>}
            </div>
          </div>;
        })}
      ...
      <div className="flex flex-col gap-3 border-t border-border-subtle pt-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs text-txt-secondary">{mergeItems.length ? `Merge ${mergeItems.length} department${mergeItems.length === 1 ? "" : "s"}, moving ${movedStaff} staff.` : "No merges selected."}</p>
        <div className="flex justify-end gap-2">
          <Button variant="secondary" onClick={onClose}>Cancel</Button>
          <Button loading={loading} disabled={invalid} onClick={() => onApply(plan.unmatched.map((item) => ({ sourceId: item.id, action: decisions[item.id]?.action ?? "KEEP", targetId: decisions[item.id]?.targetId })))}>Apply</Button>
        </div>
      </div>
    </div>
  </Dialog>;
