...
export function injectTracking(
  ...
  // Same origin the sender uses for dashboard links — see publicAppUrl's note on
  // why these must not be resolved independently.
  const appUrl = publicAppUrl().url;
  ...
  const rewritten = html.replace(/href="(https?:\/\/[^"]+)"/g, (match, url: string) => {
    ...
    return `href="${appUrl}/api/track/click/${token}"`;
  });

  const openToken = generateOpenToken(params.recipientId, params.campaignId);
  const pixel = `<img src="${appUrl}/api/track/open/${openToken}" width="1" height="1" alt="" style="display:none;width:1px;height:1px;border:0;" />`;
