...
beforeEach(async () => {
  ...
  const location = await prisma.location.create({
    data: {
      ...
      slug: `test-loc-${Date.now()}-${Math.random()}`,
      address: "1 Test St",
      city: "Testville",
      country: "Testland",
      organizationId: orgId,
  ...
  const parent = await prisma.user.create({
    data: { email: `parent-${Date.now()}-${Math.random()}@test.com`, password: "x", role: "BASE_USER", organizationId: orgId },
  });
  parentId = parent.id;
});
...
describe("submission pipeline", () => {
  it("rejects a camper who is too young at the cutoff date", async () => {
    ...
    const draft = await engine.createDraft({ camperProfileId: camper.id, yearId, locationId, actorId: parentId });
  ...
  it("auto-approves and assigns a registration number when the camp uses AUTO approval", async () => {
    await prisma.year.update({ where: { id: yearId }, data: { approvalMode: "AUTO" } });
    const camper = await makeCamper();
    const draft = await engine.createDraft({ camperProfileId: camper.id, yearId, locationId, actorId: parentId });
  ...
  it("rejects duplicate registrations for the same camper+year", async () => {
    const camper = await makeCamper();
    const first = await engine.createDraft({ camperProfileId: camper.id, yearId, locationId, actorId: parentId });
    await engine.submitRegistration({ registrationId: first.id, actorId: parentId });

    // createDraft is idempotent per camper+year, so this returns the same draft/pending row.
    const second = await engine.createDraft({ camperProfileId: camper.id, yearId, locationId, actorId: parentId });
...
describe("approval workflow", () => {
  ...
  it("does not exceed centre capacity when approving concurrently (quota=1)", async () => {
    const camperA = await makeCamper();
    const camperB = await makeCamper();

    const draftA = await engine.createDraft({ camperProfileId: camperA.id, yearId, locationId, actorId: parentId });
    const draftB = await engine.createDraft({ camperProfileId: camperB.id, yearId, locationId, actorId: parentId });
    ...
    // Exactly one should be APPROVED and the other WAITLISTED — quota is 1.
    expect(statuses).toEqual(["APPROVED", "WAITLISTED"]);

    const approvedCount = await prisma.registration.count({ where: { locationId, status: "APPROVED" } });
